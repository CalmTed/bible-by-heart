...
export const getRandomSentencesRange: (
  ...
  const numberOfSentencesToSelect =
    sentences.length === 2
      ? randomRange(minNumberOfSentences, 1)
      : randomRange(minNumberOfSentences, 2);
  if (numberOfSentencesToSelect === 0) {
    return [];
  }
  const errorGradedSentences = getErrorGradedSentences(
    sentences,
    wordsFromErrors
  );
  ...
  const rangeUpperEnd =
    sentences.length === 2
      ? 1
      : sentences.length - 1 - numberOfSentencesToSelect;
  ...
  for (let i = 0; i <= rangeUpperEnd; i++) {
    ...
    sentansesBasedOfErrorNumber = [
      ...sentansesBasedOfErrorNumber,
      ...new Array(item.errorRate + 1).fill(item.sentenceIndex)
    ];
  }
  const sentenceIndexToStart = randomItem(
    sentansesBasedOfErrorNumber
  ) as number;
...
export const createL40Test: CreateTestMethodModel = ({
  initialTest,
  passages,
  history
}) => {
  ...
  const sentenceRange =
    successStroke >= PERFECT_TESTS_TO_PROCEED - 1
      ? []
      : getRandomSentencesRange(sentences, wordsFromErrors);
