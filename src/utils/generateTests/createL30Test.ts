  ...
  if (successStroke === PERFECT_TESTS_TO_PROCEED) {
    ...
    if (
      ...
      if (Math.random() > 0.5) {
        ...
        const [startingSentanseIndex, endingSentenceIndex] =
          getRandomSentencesRange(sentences, wordsFromErrors, 1);
        ...
        const randomIndexToAdd = randomRange(
          0,
          Math.min(1, sentences.length - numberOfNeigbors)
        );
      ...
      const numberOfSeeds =
        successStroke > 0
          ? randomRange(1, Math.max(7, Math.floor(words.length / 20)))
          : randomRange(2, Math.max(3, Math.floor(words.length / 50)));
      Array(numberOfSeeds)
        .fill(0)
        .map((v, i) => {
          ...
          if (
            wordsFromErrors.filter((wfe) => !missingWords.includes(wfe.index))
              .length &&
            Math.random() > 0.5
          ) {
            //50% chance if there are some errors
            //seed from errors
            const randomNumber = randomRange(
              0,
              wordsFromErrors.reduce((ps, w) => ps + w.times, 0)
            );
            const biasedRandomWord =
              wordsFromErrors.filter(
                (s, i, arr) =>
                  arr.slice(0, i).reduce((ps, r) => ps + r.times, 0) >=
                  randomNumber
              )[0] || wordsFromErrors[0];
            const wordsToAdd = words
              ...
              .filter((w) => {
                return (
                  w.index >= biasedRandomWord.index - wordsBefore &&
                  w.index <= biasedRandomWord.index + wordsAfter
                );
              })
              .map((w) => w.index)
              .filter(
                (w) =>
                  words[w] && missingWords.filter((mw) => mw === w).length === 0
              );
            missingWords.push(...wordsToAdd);
            ...
            const wordsToAdd = words
              ...
              .filter((w) => {
                return (
                  w.index >= randomNumber - wordsBefore &&
                  w.index <= randomNumber + wordsAfter
                );
              })
              .map((w) => w.index)
              .filter(
            ...
            const mostSimularWords = [
              ...words
                .map((w, ind) => [ind, getSimularity(w, targetWord)])
                .filter((w) => w[1] < 1)
            ].sort((a, b) => b[1] - a[1]);
            const wordsToAdd = [
              randomNumber,
              ...mostSimularWords
                .slice(0, wordsBefore + wordsAfter)
                .map((w) => w[0])
            ].filter(
              (w) =>
                words[w] && missingWords.filter((mw) => mw === w).length === 0
