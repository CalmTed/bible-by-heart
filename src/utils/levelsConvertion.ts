...
export const testLevelToPassageLevel: (t: TESTLEVEL) => PASSAGELEVEL = (t) => {
    switch(t){
        ...
        default:
            logger.error(`Unknown test level while converting test level to passage level t:${t}`)
            return PASSAGELEVEL.l1
    }
} 
...
export const passageLevelToTestLevel: (t: PASSAGELEVEL) => TESTLEVEL = (t) => {
    switch(t){
        ...
        default:
            logger.error(`Unknown passage level while converting passage level to test level. T: ${t}`)
