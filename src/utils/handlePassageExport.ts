...
export const passagesToLSV: (state: AppStateModel) => string | false = (
  state
) => {
      ...
      if (PASSAGE_ROWS_TO_EXPORT.includes(key as keyof PassageModel)) {
        ...
        switch (key) {
          ...
          case "verseTranslation":
            newValue =
              state.settings.translations.filter((t) => t.id === value)[0]
                ?.name || "";
            break;
          case "tags":
            newValue = value.join(",");
        ...
        if (typeof newValue !== "string") {
          logger.error(
            `Caught non string value while encoding passages. Param: ${key}, Value: ${newValue}`
          );
          return false;
        }
...
export const arrayToLSV: (
  headersRow: string[],
  dataRows: string[][]
) => string | false = (headersRow, dataRows) => {
  ...
  const dataRowsAndHeadersLengthDifference = dataRows
    .map((row) => Math.abs(headersRow.length - row.length))
    .reduce((ps, s) => {
      return ps + s;
    }, 0);
...
export const LSVToArray: (dataString: string) => importArrayModel | false = (
  dataString
) => {
  let str = dataString;
  ...
  const dataRowsAndHeadersLengthDifference = arr
    .map((row) => Math.abs(headersRow.length - row.length))
    .reduce((ps, s) => {
      return ps + s;
    }, 0);
...
export const arrayToPassages: (
  decodedData: importArrayModel,
  state: AppStateModel
) => importPassagesListModel | false = ({ headers, data }, state) => {
  //validate:
  //data exists
  ...
  const sameColumnsNumber =
    data
      .map((row) => Math.abs(headers.length - row.length))
      .reduce((ps, s) => {
        return ps + s;
      }, 0) === 0;
    ...
    .map((passage, importedDataItemIndex) => {
      ...
      headers.map((header, headerIndex) => {
        ...
        if (typedHeader === "verseTranslation") {
          newPassage.verseTranslation =
            state.settings.translations.find(
              (translation) => translation.name === passage[headerIndex]
            )?.id || null;
        }
