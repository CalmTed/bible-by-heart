...
import { AppStateModel, PassageModel, TranslationModel } from "../models";

export const getNumberOfVersesInEnglish:
  (translations: TranslationModel[], passages: PassageModel[]) => number =
  ...
  const versesSumInEnglish = passages
    .filter(p => p.verseTranslation && translationsInEnglish.includes(p.verseTranslation))
