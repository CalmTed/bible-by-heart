...
export const getPerfectTestsNumber: (history: TestModel[], passageId: PassageModel) => number = (history, passage) => {
  const lastThreeTests = history
      .sort((a,b) => b.dateFinished - a.dateFinished)
      .filter(th => th.passageId === passage.id).slice(0,PERFECT_TESTS_TO_PRCEED)
