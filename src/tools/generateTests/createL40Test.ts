...
export const createL40Test: CreateTestMethodModel = ({ initialTest, passages, history }) => {
    ...
    const getRandomSentencesRange: (
      sentences:string[], 
        ...
        const numberOfSentencesToSelect = sentences.length === 2 ? randomRange(0,1) : randomRange(0,2);
        ...
        const rangeUpperEnd = sentences.length === 2 ? 1 : sentences.length - 1 - (numberOfSentencesToSelect - 1)
    ...
    const sentenceRange = successStroke >= PERFECT_TESTS_TO_PROCEED -1 ? [] : getRandomSentencesRange(sentences)
