...
// The version of the STATE SHAPE, not of the app. `alowedStateVersions` and
// every converter in `stateVersionConvert.ts` are written against it, so it
// moves only when the shape does - it was never meant to track package.json.
...
// place it cannot drift - which is exactly what it did for six releases while
// the About screen printed the state version instead.
...
// daily backup written by AppProvider - it is always at the CURRENT state
// version and is overwritten every 24h. STORAGE_PRECONVERT_BACKUP_NAME holds
// the raw state as it was BEFORE the very first state-version conversion this
// install ever ran; it is written once and never overwritten, so a broken
// converter (or a chain of them) can never destroy the last known-good data.
// They shared one key, which meant the daily backup silently ate the
// pre-conversion snapshot within a day of an upgrade.
...
// needs a name of its own to be nameable there. A leading control character
// keeps it out of reach of any tag a user could type.
...
// How many options a test that asks the user to PICK one has to offer. One
// option is not a test: it cannot be got wrong, and it is still recorded as a
// pass, which is what the level-up maths counts. A level that cannot reach this
// many options is not that level - it becomes the half of its level that asks
// about the address instead, which needs no library to draw decoys from.
...
// alone must not be able to hand out a level upgrade.
...
// per action; whatever is pending is flushed when the app leaves the
...
export enum SCREEN {
  ...
  //. A scrolling multi-section list is navigation, not a dialog.
...
export const VIBRATION_PATTERNS = {
  ...
  wordClick: 10,
  APSelectVerse: 10,
  // a long press that changed something - the list row collapsing
  longPress: 30
};

// Candy-UI motion vocabulary. Every reanimated surface pulls its timing from
// here so the whole app springs the same way - inventing per-component numbers
// is what makes an animated app feel assembled rather than designed. SPRING is
// deliberately slightly under-damped: the ~2% overshoot is the "candy" bounce.
// Durations stay short; anything above ~250ms reads as sluggish on the old
// small Androids this app must stay pleasant on.
export const ANIMATION = {
  ...
  riseDistance: 16,
  // How far the Header falls INTO place from above. A bar is a frame, not
  // content: it arrives from the edge it lives on rather than rising from below
  // like the screen it caps, which read as the two scrambling past each other.
  // Half of riseDistance on purpose - the frame must move LESS than what is
  // arriving underneath it, or the eye follows the wrong thing.
  ...
  // moving underneath it (the Header behind its screen transition). Short
  ...
  staggerMs: 90,
  // How far a control sinks under a finger. Deliberately smaller than
  ...
  pressScale: 0.96,
  // Gesture geometry. `swipeThreshold` is how far a finger has to travel before
  // a swipe counts as asking for a screen rather than as a twitch;
  // `pullTrigger` is the longer pull the practice arrow needs, because that
  // gesture STARTS a session and a session is not something to fall into;
...
// must stop growing rather than stretch a passage across 1800px. Same principle
// as ANIMATION: the number lives in one place, so every wide-screen surface
// stops at the same width instead of each picking its own. The app mark's
// natural size. The artwork is 255x160, so a caller picks a height and the
// width follows - the home screen scales it to the phone it is on rather than
// every screen hard-coding 255.
...
export const LAYOUT = {
  ...
  // and on the two screens that sized their scroll area in percent it WAS cut
  // off. One number, so every list ends the same way.
  scrollBottomGap: 24
};
...
export const THEME_DARK = StyleSheet.create({
  // No `paddingTop` here on purpose. It used to be a flat 30, which every
  // screen wore *in addition to* its Header's real device inset - so the bar
  // sat 30px lower than it should on a screen that had one, and screens without
  // one had 30px standing in for a status bar that is 47 on some phones and 24
  // on others. The top margin belongs to `Header`, which measures it.
  screen: {
  ...
  // The contentContainerStyle of any scrolling surface: its last row clears the
  // screen edge instead of ending flush against it. Colourless like
...
// One text source, described the way the API describes it. `sourceId` is the id
// the API addresses a translation by; a translation the user invented has none,
// and its text is typed by hand.
...
// The text sources the app knows without asking anyone. The API serves the same
// list at `API_LINK.translationCatalogue` and its answer wins when it arrives -
// but the app is offline-first, so this shipped copy is what makes a translation
...
export const BUNDLED_TRANSLATION_SOURCES: TranslationSourceModel[] = [
  ...
  {
    sourceId: "ukr-kul",
    title: "Переклад Куліша та Пулюя",
    language: LANGCODE.ua,
    isRemote: false
  },
  {
...
export enum API_LINK {
  ...
  translationCatalogue = "api/translation/getAll",
  translationPassage = "api/translation/getPassage"
}
