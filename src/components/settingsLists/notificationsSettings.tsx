...
import { checkSchedule, getAutoTimeTrigger, schedulePushNotification } from '../../tools/notifications';
...
export const NotificationsSettingsList: FC<NotificationsSettingsListModel> = ({theme, t, state, setState}) => {
    ...
    [
        ...
        JSON.stringify(state.settings.remindersList)
    ])
  ...
  return <View>
    ...
    <SettingsMenuItem
        ...
        subtext={`${t(
            ...
            state.settings.remindersSmartTime
                ? `${t("settsEnabled")} ${addZero(autoTime.hour as number)}:${addZero(autoTime.minute as number)}`
                : t('settsDisabled')
        }`}
        type="checkbox"
