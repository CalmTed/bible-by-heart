...
export const LevelPicker:FC<LevelPickerModel> = ({targetPassage, t, handleChange, handleOpen, state, activeTestObj, handleRestart}) => {
  ...
  <MiniModal theme={theme} shown={levelPickerShown} handleClose={() => setLevelPickerShown(false)}>
    <Text style={levelPickerStyles.headerText}>{t("LevelPickerHeading")}</Text>
    ...
    </View>
    {
      ...
      targetPassage.maxLevel !== PASSAGE_LEVEL.l5 &&
        <Text style={levelPickerStyles.subText}>{t("LevelPickerSubtext")}  ({getPerfectTestsNumber(state.testsHistory,targetPassage)}/{PERFECT_TESTS_TO_PRCEED})</Text>
    }
    {
      !activeTestObj &&
      targetPassage.maxLevel === PASSAGE_LEVEL.l5 &&
        <Text style={levelPickerStyles.subText}>{t("LevelPickerSubtextL5")}  ({getPerfectTestsNumber(state.testsHistory,targetPassage)})</Text>
    }
    {
      activeTestObj &&
    ...
    && <Text style={levelPickerStyles.subText}>{t("LevelPickerSubtextSecond")}</Text>}
