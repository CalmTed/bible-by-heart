    ...
    const activateTests = () => {
        setState((prevState) => {
            const newState = reduce(prevState, {
                name: ActionName.setActiveTests,
                payload: generateTests(
                    state.passages.filter(
                        (p) => !p.tags.includes(archivedName)
                    ),
                    state.testsHistory
                )
            });
                    ...
                    additionalChildren={[
                        ...
                        <View style={{ ...testsStyle.testNav }}>
                            {state.testsActive.map((t, i) => {
                                const isFinished =
                                    state.testsActive[i].isFinished;
                                const hasErrors =
                                    !!state.testsActive[i].errorNumber;
                                const isFirst = i === 0;
                                //if it first and unfinished
                                //or if not finished and previus is finished
                                const isNextUnfinished =
                                    (isFirst && !isFinished) ||
                                    (!isFinished &&
                                        state.testsActive[
                                            i - 1
                                        ]?.triesDuration.filter(
                                            (td) => td.length === 1
                                        ).length === 0);
                                const color =
                                    isFinished ||
