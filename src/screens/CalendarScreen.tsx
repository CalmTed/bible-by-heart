...
export const CalendarScreen: FC<ScreenPropsModel<SCREEN.calendar>> = ({
  ...
  const monthDaysArr = new Array(
    ...
    .fill(0)
    .map((m, i) => i + 1);
  const previusMonthLastDayNumber = Math.floor(
    (selectedMonth - 1000) / DAY / 1000
  );
  const nextMonthFirstDayNumber = Math.floor(
    (selectedMonth + monthDaysArr.length * DAY * 1000) / DAY / 1000
  );
  const selectedDayNumber = !selectedDay
    ? null
    : Math.floor(selectedDay / 1000 / DAY);
  const dayDuration = !selectedDayNumber
    ? null
    : daysStats?.[selectedDayNumber];
  const dayDetailedStats = !selectedDay
    ? null
    : getTimeBoundStats(state, selectedDay, selectedDay + DAY * 1000 - 1000);
  const selectedDayTests = !selectedDay
    ? null
    : state.testsHistory.filter(
        (h) =>
          h.td[0][0] > selectedDay &&
          h.td[0][1] < selectedDay + DAY * 1000 - 1000
      );
  const handleSetPreviusMonth = () => {
    const closetDayBefore =
      [
        ...daysStats
          .map((d, i) => i)
          .filter((i) => i < previusMonthLastDayNumber)
      ]
        .sort()
        .reverse()?.[0] || null;
    if (!closetDayBefore) {
    ...
    const monthOfTheClosestDay = new Date(
      `${closestDayBeforeDate.getFullYear()}-${closestDayBeforeDate.getMonth() + 1}-01`
    );
    setSelectedMonth(monthOfTheClosestDay.getTime());
  };
  const handleSetNextMonth = () => {
    const closetDayAfter =
      [
        ...daysStats
          .map((d, i) => i)
          .filter((i) => i >= nextMonthFirstDayNumber)
      ].sort()?.[0] || null;
    if (!closetDayAfter) {
      return;
    }
    ...
    const monthOfTheClosestDay = new Date(
      `${closestDayAfterDate.getFullYear()}-${closestDayAfterDate.getMonth() + 1}-01`
    );
    setSelectedMonth(monthOfTheClosestDay.getTime());
  };
  const handleSetPreviusDay = () => {
    ...
    const closetDayBefore =
      [...daysStats.map((d, i) => i).filter((i) => i < selectedDayNumber)]
        .sort()
        .reverse()?.[0] || null;
    if (!closetDayBefore) {
      return;
    ...
    if (
      ...
      new Date(selectedMonth).getFullYear() !==
        closestDayBeforeDate.getFullYear()
    ) {
      const monthOfTheClosestDay = new Date(
        `${closestDayBeforeDate.getFullYear()}-${closestDayBeforeDate.getMonth() + 1}-01`
      );
  ...
  const previusMonthAvalible = (() => {
    return (
      daysStats
        .filter((d, i) => i <= previusMonthLastDayNumber)
        .filter((d) => d).length > 0
    );
  })();
  ...
  const nextMonthAvalible = (() => {
    return (
      daysStats.filter((d, i) => i >= nextMonthFirstDayNumber).filter((d) => d)
        .length > 0
    );
  })();
  const previusDayAvalible = (() => {
    ...
    return (
      daysStats.filter((d, i) => i < selectedDayNumber).filter((d) => d)
        .length > 0
    );
  })();
  ...
  const monthName = t(
    `Month_${new Date(selectedMonth).getMonth() + 1}` as WORD
  );
  return (
      ...
      {!selectedDay && (
        <View style={calendarStyle.listView}>
          <View style={calendarStyle.buttons}>
            ...
            />
            <Text style={calendarStyle.buttonsTitle}>
          ...
          <PanGestureHandler onEnded={handleCalendarSwipe}>
            <Animated.View style={calendarStyle.daysList}>
              {monthDaysArr.map((day) => {
                ...
                const dayRatio =
                  (0.95 / (Math.max(...daysStats.filter((d) => d)) || 1)) *
                    (dayStats || 0) +
                  0.05;
                const weekday = new Date(selectedMonth).getDay();
                const marginLeft = weekday ? weekday - 1 : 6;
                const gradientColors =
                  state.settings.theme === THEMETYPE.light
                    ? [
                        `rgba(127,222,52,${dayRatio})`,
                        `rgba(231,223,11,${dayRatio})`
                      ]
                    : [
                        `rgba(63,111,26,${dayRatio})`,
                        `rgba(26,134,158,${dayRatio})`
                      ];
                return (
                  <LinearGradient
                    key={day}
        ...
        <View style={calendarStyle.listView}>
          ...
          {!dayDuration && (
            <View style={calendarStyle.placeHolderTextView}>
              <Text style={calendarStyle.placeHolderText}>
                {t("NoDataForThisDay")}
              </Text>
            </View>
          )}
            ...
            <PanGestureHandler onEnded={handleDaySwipe}>
              <View style={calendarStyle.masonaryListView}>
                <View style={calendarStyle.masonaryItemView}>
                  <Text style={calendarStyle.masonaryItemTitle}>
                    {dayDetailedStats.maxScore}
                  </Text>
                  <Text style={calendarStyle.masonaryItemSubtext}>
                    {t("statsAbsoluteScoreSubtext")}
                  </Text>
                </View>
                <View style={calendarStyle.masonaryItemView}>
                  <Text style={calendarStyle.masonaryItemTitle}>
                    {timeStringFromMS(dayDetailedStats.totalTime)}
                  </Text>
                  <Text style={calendarStyle.masonaryItemSubtext}>
                    {t("statsTimeSpent")}
                  </Text>
                ...
                <View style={calendarStyle.masonaryItemView}>
                  <Text style={calendarStyle.masonaryItemTitle}>
                    {dayDetailedStats.totalSessions}
                  </Text>
                  <Text style={calendarStyle.masonaryItemSubtext}>
                ...
                <View style={calendarStyle.masonaryItemView}>
                  <Text style={calendarStyle.masonaryItemTitle}>
                    {dayDetailedStats.totalPassages}
                  </Text>
                  <Text style={calendarStyle.masonaryItemSubtext}>
                    {t("statsVersesNumber")}
                  </Text>
          ...
          {dayDuration && selectedDayTests && (
            <ScrollView>
              <View style={calendarStyle.testsList}>
                {[...selectedDayTests]
                  .sort((a, b) => a.td[0][0] - b.td[0][0])
                  .map((test, i, arr) => {
                    const ifFirstOfSession = !arr
                      .slice(0, i)
                      .map((arrT) => arrT.si)
                      .includes(test.si);
                    const sessionFinishTime = Math.max(
                      ...arr
                        .filter((t) => t.si === test.si)
                        .map((t) => t.td[0][1])
                    );
                    const nextSessionStartTime =
                      arr
                        .filter(
                          (t) =>
                            t.td[0][0] > sessionFinishTime && t.si !== test.si
                        )
                        .map((t) => t.td[0][1])
                    ...
                    const targetPassage = state.passages.filter(
                      (p) => p.id === test.pi
                    )[0];
                    const tagetTranslation = state.settings.translations.filter(
                      (translation) =>
                    ...
                    const wasPromoted =
                      Math.abs(promotionTime - sessionFinishTime) <
                      Math.max(
                        MINUTE * 1000 * 5,
                        nextSessionStartTime - sessionFinishTime
                      );
                    ...
                    return (
                      <View
                        ...
                        style={{
                          ...(ifFirstOfSession
                            ? calendarStyle.testsListItemWrapperFullWidth
                            : calendarStyle.testsListItemWrapper)
                        }}
                      >
                        {ifFirstOfSession && (
                          <Text style={theme.theme.subText}>
                            {timeStringFromMS(
                              test.td[0][0] - selectedDay - timeZoneOffset
                            )}{" "}
                            -{" "}
                            {timeStringFromMS(
                              sessionFinishTime - selectedDay - timeZoneOffset
                            )}
                          </Text>
                        )}
                        <View
                        ...
                        >
                          ...
                          {wasPromoted && (
                            <Text
                              style={calendarStyle.testsListItemContentSubtext}
                            >
                              {">"}{" "}
                              {
                                (testLevelToPassageLevel(test.l) +
                                  1) as PASSAGELEVEL
                              }{" "}
                            </Text>
                          )}
                          {!!test?.en && (
                            <View
                              style={
                                calendarStyle.testsListItemContentErrorView
                              }
                            >
                              <Text
                                style={{
