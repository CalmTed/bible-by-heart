...
4. `docs/PLAN.md` — **the roadmap; take the first unchecked step.** It is also the only
   file where work is marked done — a finished step is **deleted** from it, not archived
...
- The AI writes to `docs/robotdiary.md` instead: append a dated entry every session, in
  its format — **two blocks, ≤300 characters each**: what was done, and the **friction**
  (AI errors, misreadings, redos, round trips; `none` when the session was clean).
- **History lives only in `docs/robotdiary.md`.** No other doc keeps a changelog, and a
  code comment never cites a document, a step ID or a `§` section.
...
`docs/robotdiary.md` entry appended, both blocks ✓ · both l10n files updated for new
strings ✓
